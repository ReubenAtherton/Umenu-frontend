        ...
        renderItem={({ item, index }) => (
          <View>
            <Pressable
              ...
              <View style={[styles.imageCard, index === meals.length - 1 && {}]}>
                {item.source(styles.restaurant_image_dimensions)}
              </View>
              <View style={styles.textContainer}>
                <Text className='font-pregular' style={styles.itemName}>{item.name}</Text>
...
const TextView = ({ description, allergens, rating }: any) => {
  return (
    <View>
      <Text style={modalStyles.fieldTitle}>
        Description:
      </Text>
      <Text style={modalStyles.fieldText}>
        {description}
      </Text>
      <Text style={modalStyles.fieldTitle}>
